...
\begin{frame}{Reward design --- эмпирический путь}
  ...
  \begin{enumerate}\small
    ...
    \item \textbf{MC Oracle PBRS} --- dense reward через C++ Monte-Carlo симуляцию исходов боёв. Ради него написан C++-порт combat. При росте пула значимого выигрыша не дал
...
\begin{frame}{Работа продолжается: апдейты после отчёта + Future Work}
  \begin{columns}[T]
    ...
    \begin{itemize}\small
      ...
      \item Combat engine портирован на \textbf{C++ через pybind11}: \textbf{270k+ боёв/с} (90k на сложных кейсах) --- $\sim$40$\times$ к прошлой C++-версии, $\sim$800$\times$ к Python
