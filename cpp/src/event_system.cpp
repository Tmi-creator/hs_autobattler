...
void sort_triggers(
    ...
    using SortKey = std::tuple<int, int, int, int, int32_t>;
    SortKey keys[GameConst::MAX_TRIGGERS_PER_EVENT];
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (side < 0) {
            ...
            side_priority = (side == active_side) ? 0 : 1;
        }

        keys[i] = {
            is_source_trigger ? 0 : 1,
            -static_cast<int>(t.def->priority),
            side_priority,
            slot < 0 ? 999 : static_cast<int>(slot),
            t.trigger_uid
        };
    }

    // Sort indices by pre-computed keys, then reorder triggers
    int indices[GameConst::MAX_TRIGGERS_PER_EVENT];
